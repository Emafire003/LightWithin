...
public class LuxdialogueStateArgument implements ArgumentType<DialogueProgressState> {
    public static LuxdialogueStateArgument state() {
        return new LuxdialogueStateArgument();
    }

    public static <S> DialogueProgressState getState(CommandContext<S> context, String name) {
    ...
    public DialogueProgressState parse(StringReader reader) throws CommandSyntaxException {
        ...
        try {
            DialogueProgressState state = DialogueProgressState.valueOf(targetString); // Now our actual logic.
            return state; // And we return our type, in this case the parser will consider this argument to have parsed properly and then move on.
        } catch (Exception ex) {
