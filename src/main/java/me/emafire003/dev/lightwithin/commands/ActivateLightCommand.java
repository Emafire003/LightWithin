        ...
        ServerTickEvents.END_SERVER_TICK.register(server -> {
            ...
            keyCopies.forEach(uuid -> {
                ...
                if (delay_counter.getRight() > delay_counter.getLeft()){
                    ...
                    LightWithin.activateLight(p);
                    targetDelayCounterMap.remove(uuid);

                    server.getCommandSource().sendMessage(Text.literal(LightWithin.PREFIX_MSG).formatted(Formatting.AQUA).append(Text.literal("The InnerLight of §d" + p.getName().getString() + "§e has been triggered!" ).formatted(Formatting.YELLOW)));
                    if(Config.TARGET_FEEDBACK){
                        p.sendMessage(Text.literal(LightWithin.PREFIX_MSG).formatted(Formatting.AQUA).append(Text.literal("Your InnerLight was activated by force!" ).formatted(Formatting.YELLOW)));
                    }
                    return;
                }
